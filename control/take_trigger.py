...
class TakeTrigger:
    def __init__(self, port, trig_interval, led_duration):
        assert type(trig_interval) == int or float, trig_interval
        assert type(led_duration) == int or float, led_duration
...
if __name__ == "__main__":
    ...
    parser.add_argument('--interval', '-i', help='Time interval between trigger (s).', default=1)
    parser.add_argument('--led-duration', '-l',
                        help='LED light duration (ms), values: 0.1 - 16 ms', default=1)
    ...
    print(f"Set trigger interval: {trig_interval} s, LED light duration: {led_duration/10} ms")
